...
uint8_t no2[] = {0xA2, 0x20, 0xA6, 0x16, 0xB0, 0xB2}; // esp number 2 - slave
//uint8_t no3[] = {0x1A, 0xFE, 0x34, 0xEE, 0xCD, 0x15}; // esp number 3 - slave
uint8_t no3[] = {0xA2, 0x20, 0xA6, 0x16, 0xB0, 0x90}; // esp number 3 - slave
//uint8_t no4[] = {0x5E, 0xCF, 0x7F, 0x9, 0x9A, 0xFE}; // esp number 4 - slave
uint8_t no4[] = {0x5E, 0xCF, 0x7F, 0x16, 0xF9, 0xC8}; // esp number 4 - slave
...
//***GPS READING FUNCTION. return as string***//
String displayInfo()
 {
   String string;
   if (gps.location.isValid())
   {
    ...
    dtostrf(gps.location.lat(), 8, 7, ED.latitude);
    dtostrf(gps.location.lng(), 8, 7, ED.longitude);
    dtostrf(gps.time.value(), 8, 7, ED.timegps);

    //ED.latitude = gps.location.lat();
    //ED.longitude = gps.location.lng();
    //ED.timegps = gps.time.value();

   Serial.println(ED.latitude);
   Serial.println(ED.longitude);
   Serial.println(ED.timegps);
...
void loop() {
  ...
  while (Serial.available() > 0){
    if (gps.encode(Serial.read())){
     ...
     if(gps.time.centisecond() != timegps_ant){
      Serial.println("DIFF");
      esp_now_send( NULL, data, len);
      timegps_ant  = gps.time.centisecond(); 
     }
     //strncpy(timegps_ant, ED.timegps, 20);
    }
  }
